...
| v2.3.1 | 導覽列改為固定貼齊視覺視窗底部，移除 `safe-area-inset-bottom` 對高度與內距的參與，並刪除額外的 body 底部補色層；根節點改回固定 100% 高度。 |
| **v2.3.2** | 依原生 App 導覽參考重新定義貼齊方式：最外層改用 `100lvh` 完整螢幕高度，所有頁面、背景與導覽改為在同一完整螢幕容器中絕對定位，使圖示與標籤以下方實體螢幕為基準，不再停在瀏覽器縮短後的動態內容區。 |
